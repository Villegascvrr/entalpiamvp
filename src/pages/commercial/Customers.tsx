...
const getCustomerStatus = (id: string): 'active' | 'inactive' | 'risk' => {
    ...
    const statuses: ('active' | 'inactive' | 'risk')[] = ['active', 'active', 'active', 'inactive', 'risk'];
    return statuses[charCode % statuses.length];
};

const StatusBadge = ({ status }: { status: 'active' | 'inactive' | 'risk' }) => {
    switch (status) {
        case 'active':
            return <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 text-[10px] h-5">Activo</Badge>;
        case 'inactive':
            return <Badge variant="outline" className="bg-slate-500/10 text-slate-500 border-slate-500/20 text-[10px] h-5">Inactivo</Badge>;
        case 'risk':
            return <Badge variant="outline" className="bg-amber-500/10 text-amber-600 border-amber-500/20 text-[10px] h-5">Riesgo</Badge>;
    }
};
...
export default function Customers() {
    ...
    const activeCount = filteredCustomers.filter(c => getCustomerStatus(c.id) === 'active').length;
                ...
                <div className="flex-none flex items-center justify-between px-6 py-4 bg-muted/30 border-b border-border/60">
                    <div>
                        <div className="flex items-center gap-3">
                            ...
                            <Badge variant="secondary" className="text-[10px] px-1.5 h-5 font-normal bg-sky-500/10 text-sky-700 border-sky-200">
                                {activeCount} activos
                            </Badge>
                        </div>
                    ...
                    <Button onClick={() => navigate("/commercial/customers/new")} size="sm" className="gap-2 shadow-sm">
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={7} className="h-[400px] text-center">
                                            <div className="flex flex-col items-center justify-center max-w-sm mx-auto">
                                                ...
                                                <h3 className="text-lg font-semibold text-foreground">No hay clientes todavía</h3>
                                                <p className="text-sm text-muted-foreground mt-1 mb-6 text-center">
                                                    Comienza añadiendo el primer cliente a tu cartera para gestionar pedidos y ofertas.
                                                </p>
                                                <Button onClick={() => navigate("/commercial/customers/new")} className="gap-2">
                                    ...
                                    filteredCustomers.map((customer) => {
                                        ...
                                        return (
                                            <TableRow
                                                ...
                                                onClick={() => navigate(`/commercial/customers/${customer.id}`)}
                                            >
                                                ...
                                                <TableCell>
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end">
                                                            ...
                                                            <DropdownMenuItem onClick={() => navigate(`/commercial/customers/${customer.id}`)}>
                                                                Ver Detalles
                                                            </DropdownMenuItem>
                                                            <DropdownMenuItem onClick={() => navigate(`/order/new?customer=${customer.id}`)}>
                                                                Crear Pedido
                                                            </DropdownMenuItem>
                                                        </DropdownMenuContent>
                                                    </DropdownMenu>
