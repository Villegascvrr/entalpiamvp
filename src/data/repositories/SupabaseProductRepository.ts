...
export class SupabaseProductRepository implements ProductRepository {
    ...
    private async _resolveDiscount(session: ActorSession): Promise<number> {
        if (session.role === 'customer' || session.role === 'commercial' || session.role === 'admin') {
    ...
    private _mapProducts(data: any[], session: ActorSession, discountPercentage: number): Product[] {
