...
import type { Order, RecentOrder, OrderSummary, OrderStatus, OrderTimelineEvent } from "@/data/types";
...
export interface OrderRepository {
    ...
    getCustomerHistory(session: ActorSession, customerId: string): Promise<OrderSummary[]>; // Keeping for backward compat
    getOrderTimeline(session: ActorSession, orderId: string): Promise<OrderTimelineEvent[]>;
...
export class MockOrderRepository implements OrderRepository {
    ...
    async getOrderTimeline(session: ActorSession, orderId: string): Promise<OrderTimelineEvent[]> {
    ...
    async createOrder(session: ActorSession, orderData: Partial<Order>): Promise<Order> {
        ...
        const newOrder: Order = {
            ...
            date: new Date().toLocaleDateString("es-ES", { day: "2-digit", month: "2-digit", year: "numeric" }) + " 09:00",
    ...
    private _generateTimeline(order: Order): OrderTimelineEvent[] {
        ...
        events.push({
            ...
            notes: "Salida de almacén - Agencia Transportes XYZ. Tracking: 1Z999AA10123"
        });
        ...
        events.push({
            ...
            notes: "Entregado en destino. Firmado: " + order.customer.name.split(" ")[0]
        });
