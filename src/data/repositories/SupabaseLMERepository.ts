...
export class SupabaseLMERepository implements LMERepository {
    ...
    async getLatestPrice(session: ActorSession): Promise<LMEPrice | null> {
        ...
        if (error) {
            ...
            console.error("[SupabaseLMERepo] Error fetching latest price:", error.message);
            return null;
        }
    ...
    async getPriceByDate(session: ActorSession, date: string): Promise<LMEPrice | null> {
        ...
        if (error) {
            if (error.code === 'PGRST116') return null;
            console.error("[SupabaseLMERepo] Error fetching price by date:", error.message);
            return null;
        }
    ...
    async setManualPrice(session: ActorSession, price: number): Promise<LMEPrice> {
        ...
        if (error) {
            console.error("[SupabaseLMERepo] Error setting manual price:", error.message);
