...
export function DevDebugPanel() {
    ...
    const totalOrders = (adminOrders?.length || 0) + (recentOrders?.length || 0);
        ...
        <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end pointer-events-none">
            ...
            `}>
                ...
                >
                    ...
                    {isExpanded && <span className="text-xs font-bold text-emerald-400 uppercase tracking-wider">RLS Debugger</span>}
                        ...
                        <div className="space-y-1">
                            ...
                            {session ? (
                                <div className="grid grid-cols-[60px_1fr] gap-x-2 gap-y-1">
                                    ...
                                    <span className="font-mono text-blue-300 truncate" title={session.email}>{session.email}</span>

                                    <span className="text-slate-500">Role:</span>
                                    <span className="font-mono text-purple-300">{session.role}</span>

                                    <span className="text-slate-500">Tenant:</span>
                                    <span className="font-mono text-orange-300 truncate" title={session.tenantId}>
                        ...
                        <div className="space-y-1 pt-2 border-t border-white/10">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div className="bg-white/5 p-1.5 rounded text-center">
                                    <span className="block text-[10px] text-slate-500">Admin Orders</span>
                                    <span className="font-bold text-white">{adminOrders?.length || 0}</span>
                                </div>
                                <div className="bg-white/5 p-1.5 rounded text-center">
                                    <span className="block text-[10px] text-slate-500">My Orders</span>
                                    <span className="font-bold text-white">{recentOrders?.length || 0}</span>
                                </div>
                            </div>
                            {isLoading && (
                                <p className="text-[10px] text-yellow-400 animate-pulse mt-1">Fetching data...</p>
                            )}
                        </div>
