  ...
  return (
    <div className="flex flex-col h-full w-full max-h-full overflow-hidden bg-card border-l border-border shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)]">
      ...
      <div className="px-4 py-3 border-b border-border bg-muted/30 flex items-center justify-between shrink-0">
        ...
        <Badge
          variant="outline"
          className="text-[9px] font-mono h-5 px-1.5 border-dashed"
        >
          {items.length} ITEMS
        </Badge>
      </div>
        ...
        {items.length === 0 ? (
          ...
          <div className="divide-y divide-border/40">
            {items.map((item) => (
              <div
                key={item.id}
                className="p-3 hover:bg-muted/10 transition-colors group relative"
              >
        ...
        <div className="pt-1 space-y-2">
          ...
          <Button
            className={cn(
              ...
              items.length > 0
                ? "bg-primary hover:bg-primary/90 hover:shadow-lg hover:scale-[1.01]"
