...
function createProductRepository(): ProductRepository {
  if (appConfig.mode !== "demo") {
    ...
    const ready = import("./SupabaseProductRepository")
      .then((m) => {
        impl = new m.SupabaseProductRepository();
      })
      .catch((err) => {
        console.error(
          "[RepoFactory] Failed to load SupabaseProductRepository, falling back to mock:",
          err,
        );
    ...
    return {
      getProducts: async (s) => {
        await ready;
        return impl!.getProducts(s);
      },
      getCategories: async (s) => {
        await ready;
        return impl!.getCategories(s);
      },
      getProductsByCategory: async (s, c) => {
        await ready;
...
function createOrderRepository(): OrderRepository {
  if (appConfig.mode !== "demo") {
    ...
    const ready = import("./SupabaseOrderRepository")
      .then((m) => {
        impl = new m.SupabaseOrderRepository();
      })
      .catch((err) => {
        console.error(
          "[RepoFactory] Failed to load SupabaseOrderRepository, falling back to mock:",
          err,
        );
    ...
    return {
      getAdminOrders: async (s) => {
        await ready;
        return impl!.getAdminOrders(s);
      },
      getRecentOrders: async (s) => {
        await ready;
        return impl!.getRecentOrders(s);
      },
      getCustomerHistory: async (s, c) => {
        await ready;
        return impl!.getCustomerHistory(s, c);
      },
      createOrder: async (s, d) => {
        await ready;
        return impl!.createOrder(s, d);
      },
      validateOrder: async (s, id) => {
        await ready;
        return impl!.validateOrder(s, id);
      },
      updateOrderStatus: async (s, id, st) => {
        await ready;
        return impl!.updateOrderStatus(s, id, st);
      },
      getActiveOrders: async (s) => {
        await ready;
        return impl ? impl.getActiveOrders(s) : [];
      },
      getArchivedOrders: async (s) => {
        await ready;
        return impl ? impl.getArchivedOrders(s) : [];
      },
      getOrderTimeline: async (s, id) => {
        await ready;
        return impl ? impl.getOrderTimeline(s, id) : [];
      },
    };
  }
...
function createCustomerRepository(): CustomerRepository {
  try {
    if (appConfig.mode !== "demo") {
      ...
      const ready = import("./SupabaseCustomerRepository")
        .then((m) => {
          impl = new m.SupabaseCustomerRepository();
        })
        .catch((err) => {
          console.error(
            "[RepoFactory] Failed to load SupabaseCustomerRepository:",
            err,
          );
        });

      return {
        getCustomers: async (s) => {
          await ready;
          return impl ? impl.getCustomers(s) : [];
        },
        getCustomerById: async (s, id) => {
          await ready;
          return impl ? impl.getCustomerById(s, id) : null;
        },
        createCustomer: async (s, d) => {
          await ready;
          if (!impl) throw new Error("Repo not loaded");
          return impl.createCustomer(s, d);
        },
        updateCustomer: async (s, id, d) => {
          await ready;
          if (!impl) throw new Error("Repo not loaded");
          return impl.updateCustomer(s, id, d);
        },
      };
    }
...
function createLMERepository(): LMERepository {
  try {
    if (appConfig.mode !== "demo") {
      ...
      const ready = import("./SupabaseLMERepository")
        .then((m) => {
          impl = new m.SupabaseLMERepository();
        })
        .catch((err) => {
          console.error(
            "[RepoFactory] Failed to load SupabaseLMERepository:",
            err,
          );
        });

      return {
        getLatestPrice: async (s) => {
          await ready;
          return impl ? impl.getLatestPrice(s) : null;
        },
        getPriceByDate: async (s, d) => {
          await ready;
          return impl ? impl.getPriceByDate(s, d) : null;
        },
        setManualPrice: async (s, p) => {
          await ready;
          if (!impl) throw new Error("Repo not loaded");
          return impl.setManualPrice(s, p);
        },
        getHistory: async (s, l) => {
          await ready;
          return impl ? impl.getHistory(s, l) : [];
        },
      };
    }
